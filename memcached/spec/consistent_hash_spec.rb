require File.dirname(__FILE__) + "/../lib/client/consistent_hash"

describe ConsistentHash do
  before(:each) do
    @servers = {:s1 => "s1", :s2 => "s2", :s3 => "s3", :s4 => "s4", :s5 => "s5", :s6 => "s6"}
    @servers.each do |n, s|
      ConsistentHash.add_server(n,s)
    end
  end
  ...
  it "should not change more than 75% of keys" do
    ...
    before = keys.map() { |k| ConsistentHash.get(k)}
    
    ConsistentHash.add_server(:s7, "s7")
    
    after = keys.map() { |k| ConsistentHash.get(k)}
  ...
  it "should distribute keys evenly accross servers" do
    ...
    servers = keys.map() { |k| ConsistentHash.get(k)}
